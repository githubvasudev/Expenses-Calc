(function () {
  ...
  function TransactionListCtrlFunction ($http, $rootScope, transactionService) {
    ...
    // }
    vm.transactions = $rootScope.transactions;
  }
  
  ExpensesDetailCtrlFunction.$inject = ["$scope", "$rootScope","transactionService"];
  function ExpensesDetailCtrlFunction ($scope, $rootScope,transactionService) {
     var edd = this;
    $scope.date= new Date();
    $scope.mates = [];
    edd.selected=[];
    ...
    $scope.submitExpenses = function(){
      ...
      var expenseObj = {date:$scope.date,amount:$scope.amount,givenBy:$scope.givenBy.name,givenTo:edd.givenTo,Vasu:vasuShare,Mani:maniShare,Suri:suriShare,Arun:arunShare,reason:$scope.reason,other:$scope.other};
      edd.transactions.push(expenseObj);
    ...
    transactionService.getList().then(function (data) {
      $rootScope.transactions.push(expenseObj);
